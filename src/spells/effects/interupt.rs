...
pub fn interupt_effect_apply(
    time: Res<Time>,
    damage_query: Query<(Entity, &InteruptEffect, &EffectTarget), With<EffectMarker>>,
    mut char_query: Query<(&CharacterIndex, &mut CharacterCastState, &mut InteruptState)>,
    mut commands: Commands,
) {
    for (effect_entity, interupt_effect, effect_target) in damage_query.iter() {
            ...
            .find(|(index, _, _)| effect_target == *index)
        {
            ...
            if let Some(cast) = character_cast {
                ...
                tracing::info!("attempted interupt");
            }
        }
    }
}
